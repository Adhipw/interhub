    ...
    <div class="space-y-10">
      ...
      <div v-else class="space-y-6">
        <Card v-for="event in events.data" :key="event.id" class="p-8 border-none shadow-sm flex items-start gap-6 relative overflow-hidden group">
          <div 
            class="absolute top-0 left-0 w-1.5 h-full transition-colors group-hover:w-2"
            :class="{
          ...
          <div class="flex-1 space-y-4">
             <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                   ...
                   <div class="flex items-center gap-4 text-xs text-slate-500">
                      <span class="flex items-center gap-1.5 font-medium" :class="event.severity === 'high' ? 'text-red-500' : ''">
                ...
                <div class="flex items-center gap-3">
                   <div class="px-3 py-1.5 bg-slate-50 dark:bg-slate-900 rounded-xl flex flex-col gap-1 text-xs font-bold text-slate-500 font-medium border border-slate-100 dark:border-slate-800">
             ...
             <div v-if="event.payload" class="p-4 bg-slate-900 rounded-xl overflow-hidden group/payload relative">
                <p class="text-xs font-bold text-slate-500 font-medium mb-2">{{ t('admin.security.payload') }}</p>
