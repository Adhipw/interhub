    ...
    <DashboardLayout>
        <div class="space-y-12 pb-20">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 animate-fade-in">
                <div class="space-y-4">
                    <div class="inline-flex items-center gap-2 px-3 py-1 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-full text-xs font-semibold tracking-wide border border-red-100 dark:border-red-900/30">
                        <Lock class="w-3 h-3" />
                ...
                <div class="flex items-center gap-4 bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                    <div class="flex items-center gap-3 px-4 py-2 border-r border-slate-100 dark:border-white/5">
                        ...
                        <span class="text-xs font-semibold tracking-wide text-slate-500">System Live</span>
                    </div>
...
v-for="(stat, i) in [
                ...
                ]" :key="i" class="!p-8 !rounded-2xl group border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-[box-shadow] duration-200">
                    ...
                    <template v-else>
                        <div :class="['w-14 h-14 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-200', stat.bg, stat.color]">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div class="lg:col-span-2 space-y-8">
                    <div class="flex items-center justify-between px-4">
                        ...
                        <Link href="/admin/audit-logs" class="text-xs font-medium text-blue-600 hover:underline">View All Logs</Link>
                    </div>

                    <Card class="!p-0 !rounded-2xl overflow-hidden border-slate-200 dark:border-slate-800 shadow-sm">
                            ...
                            <table class="w-full text-left">
                                <thead>
                                    <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-white/5">
                                        <th class="px-8 py-6 text-xs font-medium text-slate-400">Admin</th>
                                        <th class="px-8 py-6 text-xs font-medium text-slate-400">Aktivitas</th>
                                        <th class="px-8 py-6 text-xs font-medium text-slate-400 text-right">Waktu</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50 dark:divide-white/5">
                                    <tr v-for="log in recentLogs" :key="log.id" class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                        ...
                                        <td class="px-8 py-6">
                                            <p class="text-xs font-medium text-slate-600 dark:text-slate-400 leading-relaxed">{{ log.description || log.action }}</p>
                                        </td>
                                        <td class="px-8 py-6 text-right">
                                            <span class="text-xs font-medium text-slate-400">{{ log.created_at_human }}</span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="space-y-12">
                    ...
                    <section class="space-y-6">
                        <div class="flex items-center justify-between px-4">
                            ...
                            <span class="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded-lg">{{ pendingCompanies.length }}</span>
                        </div>
                        ...
                        <div v-else-if="pendingCompanies.length > 0" class="space-y-4">
                            <div v-for="company in pendingCompanies" :key="company.id" class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm group hover:shadow-md transition-[box-shadow]">
                                <div class="flex items-center gap-4 mb-6">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs font-medium text-slate-400">{{ company.created_at_human }}</p>
                                    </div>
                                </div>
                                <Link 
                                    :href="'/admin/companies?search=' + company.name"
                                    class="w-full block py-3.5 bg-slate-50 dark:bg-slate-800 text-center rounded-2xl font-bold text-sm hover:bg-blue-600 hover:text-white active:scale-[0.98] transition-colors cursor-pointer"
                                >
                                    Review Perusahaan
                                </Link>
                            </div>
                        ...
                        <div v-else class="text-center py-12 bg-slate-50/50 dark:bg-slate-800/20 rounded-2xl border-2 border-dashed border-slate-100 dark:border-slate-800">
                            ...
                            <p class="text-xs font-medium text-slate-400">Semua aman & terverifikasi.</p>
                        </div>
                    </section>

                    <!-- Security Alert -->
                    <div class="bg-red-600 p-8 rounded-2xl text-white shadow-md relative overflow-hidden group">
                        ...
                        <Link href="/admin/security-guide" class="inline-flex items-center gap-2 bg-white text-red-600 px-6 py-3 rounded-xl font-bold text-sm hover:bg-red-50 active:scale-[0.98] transition-colors cursor-pointer shadow-sm">
                            Baca Panduan
                        </Link>
                    </div>
                </div>
